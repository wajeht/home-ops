...
| DNS                | wildcard CNAME (`*.wajeht.com → tunnel`) — [external-dns](https://github.com/kubernetes-sigs/external-dns) only if jaw.dev migrates without the tunnel | One CF record covers every app; adding a subdomain needs zero DNS work                                                                                     |
