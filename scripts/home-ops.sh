...
cmd_install() {
    ...
    create_secret authelia_smtp_host "$(sops -d infra/authelia/.enc.env 2>/dev/null | grep "^SMTP_HOST=" | cut -d= -f2 || true)"
    create_secret authelia_smtp_username "$(sops -d infra/authelia/.enc.env 2>/dev/null | grep "^SMTP_USERNAME=" | cut -d= -f2 || true)"
