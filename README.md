...
All containers are hardened: capabilities dropped (`cap_drop: ALL`), privilege escalation disabled (`no-new-privileges`), with only required capabilities added back per-service. [Borgmatic](https://torsion.org/borgmatic/) handles automated backups — 2 critical apps hourly, rest daily — with database dumps (8 Postgres + 19 SQLite), weekly integrity checks, and ntfy notifications.
